...
function salvar() {
    ...
    if (id && nome && modelo && marca && ano) {// se tudo certo 
        switch (oQueEstaFazendo) {
            case 'inserindo':
                carro = new Carro(id, nome, modelo, marca, ano);
                listaCarros.push(carro);
                mostrarAviso("Inserido na lista");
                break;
            case 'alterando':
                carroAlterado = new Carro(id, nome, modelo, marca, ano);
...
function prepararESalvarCSV() { //gera um arquivo csv com as informações da lista. Vai enviar da memória RAM para dispositivo de armazenamento permanente.
    ...
    let textoCSV = "";
     let fimDeLinha = "\n";
    for (let i = 0; i < listaCarros.length; i++) {
        const linha = listaCarros[i]; //variavel linha contem as informações de cada carro
         if (i == listaCarros.length - 1) {
            fimDeLinha = "";
        }
        textoCSV += linha.id + ";" +
            linha.nome + ";" +
...
// Função para processar o arquivo CSV e transferir os dados para a listaCarros
function converterDeCSVparaListaObjeto(arquivo) {
    ...
    leitor.onload = function (e) {
        ...
        listaCarros = []; // Limpa a lista atual (se necessário)
        for (let i = 0; i < linhas.length; i++) {
            ...
            if (linha) { //verifica se a linha não está vazia
                ...
                if (dados.length === 2) { //verifica os seis campos
                    // Adiciona os dados à listaCarros como um objeto
                    listaCarros.push({
                        id: dados[0],
                        nome: dados[1],
                        modelo: dados[2],
